import { TextRootTemplateMapper, TextSplitter } from "../../../../src/common/convert/input/text/text_splitter.js";
...
describe('TextSplitter Tests', () => {
  const rootTemplate: TextRootTemplateMapper = (text: string): string => `
  ...
  const rootTemplateMinimized = (text: string): string => SsmlMinimizer.minimize(rootTemplate(text));

  test('should split text at period before split limit', () => {
    const text = "This is a test. Here is another sentence. This is yet another sentence.";
    const options = new ConvertTextOptionsGoogle({ splitLimit: 250 });

    const result = TextSplitter.split(text, rootTemplate, options);

    result.forEach((chunk) => {
      expect(chunk.length).toBeLessThanOrEqual(options.splitLimit);
    });
    expect(result).toEqual([
      rootTemplateMinimized("This is a test. Here is another sentence."),
      rootTemplateMinimized("This is yet another sentence."),
    ]);
  });

  test('should split text at comma if no period found', () => {
    const text = "This is a test, and it has a comma, no period, yet another clause.";
    const options = new ConvertTextOptionsGoogle({ splitLimit: 250 });
  ...
  test('should split text at the split limit if no period or comma is found', () => {
    const text = "This is a long sentence with no commas or periods that needs to be split.";
    const options = new ConvertTextOptionsGoogle({ splitLimit: 250 });
  ...
  test('should not split text if it fits within the split limit', () => {
    ...
    const resultContent = [
      "This is a short sentence that fits the limit.",
    ];
  ...
  test('should handle text with multiple periods and commas', () => {
    const text = "This is a test. Another test, with more commas. Finally, the last sentence.";
    const options = new ConvertTextOptionsGoogle({ splitLimit: 250 });
  ...
  test('should handle long continuous text with no periods or commas', () => {
    const text = "ThisIsALongContinuousTextWithoutAnyPeriodsOrCommasSoItShouldSplitAtTheLimit";
    const options = new ConvertTextOptionsGoogle({ splitLimit: 250 });
  ...
  test('should handle text that ends with a period or comma', () => {
    const text = "This sentence ends with a period. Another sentence ends with a comma,";
    const options = new ConvertTextOptionsGoogle({ splitLimit: 250 });
  ...
  test('should handle text with a mix of short and long sentences', () => {
    const text = "Short. This is a much longer sentence that might need splitting. Another short one.";
    const options = new ConvertTextOptionsGoogle({ splitLimit: 250 });
