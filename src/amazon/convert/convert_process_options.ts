...
export class ConvertProcessOptionsAmazon {
  ...
  constructor({
    ...
    // Enhanced options with Amazon-specific defaults
    ...
    this.rateLimitOptions = {
      enabled: false,
      algorithm: 'token-bucket', // Amazon works well with token bucket
      customConfig: ProviderRateLimits.amazon,
      ...rateLimitOptions
    };
  ...
  /**
   * Get Amazon-specific rate limit configuration
   */
  getRateLimitConfig(): RateLimitConfig | undefined {
    if (!this.rateLimitOptions?.enabled) {
      return undefined;
    }
    return this.rateLimitOptions.customConfig || ProviderRateLimits.amazon;
  }
